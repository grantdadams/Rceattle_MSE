...
# * Adjust f-prop ----
avg_F <- (exp(ms_mod_ricker$estimated_params$ln_mean_F+ms_mod_ricker$estimated_params$F_dev)) # Average F from last 2 years
avg_F <- rowMeans(avg_F[,(ncol(avg_F)-2) : ncol(avg_F)])
f_ratio <- avg_F[14:16]
f_ratio <- f_ratio/sum(f_ratio)

# Adjust future F proportion to each fleet
ms_mod_ricker$data_list$fleet_control$proj_F_prop <- c(rep(0, 7), 1,0,0,1, 0,0, f_ratio, 0, 0)
ms_mod_ricker$estimated_params$proj_F_prop <- ms_mod_ricker$data_list$fleet_control$proj_F_prop
...
nsim = 1; start_sim = 1; assessment_period = 1; sampling_period = 1; simulate_data = TRUE; regenerate_past = FALSE; sample_rec = TRUE; rec_trend = 0; fut_sample = 1; cap = NULL; catch_mult = NULL; seed = 666; regenerate_seed = seed; loopnum = 1; file = NULL; dir = NULL; timeout = 30

msecheck <- mse_run_parallel(
  om = ms_mod_ricker, 
  em = ms_run_concmsy, 
  nsim = 1, 
  start_sim = 1, 
...
if(regenerate_past){
  ...
  em <- fit_mod(
    ...
    suit_meanyr = em$data_list$suit_meanyr,
