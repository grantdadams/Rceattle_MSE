...
for(om in 1:length(om_list)){  # OM model
  for(em in 1:length(em_hcr_list)){ # EM and HCR
    ...
    # STEP 1 -- Load MSE
    mse3 <- load_mse(dir = paste0("Runs/GOA1977/", om_names[om],"/", em_hcr_names[em],"/ConstantR/No cap"), file = NULL)
    ...
    write.csv(mse_metrics, file = paste0("Results/Tables/GOA/GOA_table_", MSE_names,".csv"))
    ...
    plot_depletionSSB(mse3$Sim_1$EM, mse = FALSE, incl_proj = TRUE, file = paste0("Results/Figures/Depletion/GOA Perceived 1-Sim ", MSE_names), species = c(1,3,2), width = 4.3, height = 4)
