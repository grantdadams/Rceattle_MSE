...
om_names_print = c("1. No climate", "2. w/ SRR", 
                   "3. 1-2.6", "4. w/ SRR", 
                   "5. 2-4.5", "6. w/ SRR", 
                   "7. 5-8.5", "8. w/ SRR"
)
...
# * EM names ----
EM_names <- paste0(c("ss_run_Tier3", "ss_run_dynamicTier3", "ss_run_M_Tier3", "ss_run_M_dynamicTier3", "ms_run_fb40iter", "ms_run_fb40", "ms_run_cmsy", "ms_run_concmsy"), "_EM")

HCR_names_print <-  (c("NPFMC Fix-M", "NPFMC Fix-M (Dynamic B0)", "NPFMC Est-M" , "NPFMC Est-M (Dynamic B0)", "MS-B40 iter", "MS-B40 simul", "MS-MSY", "MS-cMSY"))
...
# - PMs
performance_metrics <- c("Average Catch", "Catch IAV", # "Avg terminal SSB Relative MSE" ,
                         "OM: P(SSB < SSBlimit)" , "OM: Terminal SSB", "OM: Terminal SSB Depletion (Dynamic)",
                         "OM: Terminal Dynamic SB0") # Names in table


performance_metrics_print <- c("average annual catch (1,000 mt)", 
                               ...
                               "terminal spawning stock biomass (1,000 mt)",
                               "terminal spawning stock biomass depletion relative to dynamic SB0",
                               "terminal dynamic SB0 (1,000 mt)")
...
for(sp in 1:3){
  for(pm in 1:length(performance_metrics)){
    ...
    # - Write table
    print(paste0("**Supplementary Table S3.", 2+sp, ".PM-",pm,".** Summary of performance metric ", pm," (", performance_metrics_print[pm], ") across OMs for **", species,"**."))
